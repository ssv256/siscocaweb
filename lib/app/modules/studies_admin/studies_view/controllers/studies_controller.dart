...
class StudyAdminController extends GetxController {
  final repo.StudyRepository repository;
  
  StudyAdminController({required this.repository});
  ...
  /// Removes a study by ID
  Future<void> removeData(int id) async {
    isLoading.value = true;
    try {
      await repository.deleteStudy(id);
      await _loadStudies();
      Get.snackbar(
        'Success',
      ...
      Get.snackbar(
        'Error',
        'Failed to delete study: ${e.toString()}',
        snackPosition: SnackPosition.BOTTOM,
      );
    } finally {
      isLoading.value = false;
